...
(function () {
  ...
  /* ---- per-tool icons ----
   * The popular row and the search results were both drawing the CATEGORY
   * glyph, so the first page anyone sees was a column of identical squares
   * while every built page under it had per-tool icons. The answers come from
   * data/tool-icons.js, generated by build.js from the same rules — no second
   * copy of the resolver in the browser and nothing to keep in step.
   * If the file has not loaded, this falls back to the category glyph rather
