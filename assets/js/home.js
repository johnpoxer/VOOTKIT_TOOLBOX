...
(function () {
  ...
  function toolHref(t) { return 'tools/' + t.id + '.html'; }
  ...
  function render(list, query) {
    ...
    if (!list.length) {
      results.innerHTML = '<p class="note" style="padding:var(--s-4)">No tool matches “' + esc(query) +
        '”. Try a simpler word — or <a href="tools/" style="color:var(--accent);font-weight:600">browse all tools</a>.</p>';
    ...
    results.innerHTML = list.map(function (t, i) {
      var cat = window.VK.category(t.cat) || { name: '' };
      var soon = t.status !== 'live';
      var badge = t.processing === 'network'
      ...
      return '<a class="res" href="' + toolHref(t) + '" role="option" id="res-' + i + '" aria-selected="false">' +
               '<span class="res-main">' +
                 '<strong>' + esc(t.name) + (soon ? ' <em class="soon">coming soon</em>' : '') + '</strong>' +
                 '<span class="res-desc">' + esc(t.desc) + '</span>' +
               '</span>' +
               '<span class="res-meta">' + badge + '<span class="res-cat">' + esc(cat.name) + '</span></span>' +
             '</a>';
    }).join('');
  }
  ...
  function paintActive() {
    ...
    items.forEach(function (n, i) {
      ...
      // cosmetic only — never let it break the a11y state below
  ...
  function run() {
    ...
    render(q ? window.VK.search(q, 12) : [], q);
  }
  ...
  // stat counts from the catalog — never hardcoded
  var c = window.VK.counts;
  document.querySelectorAll('[data-count="live"]').forEach(function (n) { n.textContent = c.live; });
  document.querySelectorAll('[data-count="cats"]').forEach(function (n) { n.textContent = window.VK.CATEGORIES.length; });

  // render category grid
  var catGrid = document.getElementById('cats');
  if (catGrid) {
    catGrid.innerHTML = window.VK.CATEGORIES.map(function (cat) {
      var n = window.VK.byCategory(cat.slug).filter(function (t) { return t.status === 'live'; }).length;
      var total = window.VK.byCategory(cat.slug).length;
      return '<a class="card tool-card" href="c/' + cat.slug + '.html">' +
        '<span class="ic">' + icon(cat.icon) + '</span>' +
        '<h3>' + esc(cat.name) + '</h3>' +
        '<p>' + esc(cat.blurb) + '</p>' +
        '<span class="res-cat">' + (n || total) + ' tools</span></a>';
    }).join('');
  }
  ...
  function icon(name) {
    ...
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">' + (p[name] || p.file) + '</svg>';
  }
})();
