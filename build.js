...
/* Paint only a non-sensitive signed-in UI hint before CSS loads. Supabase
   still verifies/restores the real session in auth.js; this prevents the
   server-rendered "Start Free" state flashing for a second on every page. */
function authHintHead() {
  return `<script>(function(d,w){try{var stores=[w.localStorage,w.sessionStorage];for(var s=0;s<stores.length;s++){var x=stores[s];if(!x)continue;for(var i=0;i<x.length;i++){var k=x.key(i);if(!/^sb-.*-auth-token$/.test(k))continue;var v=JSON.parse(x.getItem(k)||'null');if(v&&v.access_token&&v.user){d.documentElement.classList.add('vk-signed-in');return}}}}catch(e){}})(document,window);</script>`;
}
